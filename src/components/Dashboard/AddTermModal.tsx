...
interface AddTermModalProps {
  isOpen: boolean;
  onClose: () => void;
  onAdd: (term: string, correction?: string) => void;
}

export default function AddTermModal({ isOpen, onClose, onAdd }: AddTermModalProps) {
    ...
    <AnimatePresence>
      ...
      >
        ...
        >
          ...
          <h2 className="text-2xl font-bold mb-2">Add a term</h2>
          <p className="text-gray-500 mb-6 text-sm">Help Ikor recognize the words that matter to you. Add names, slang, or custom terms.</p>
          ...
          <button
            onClick={() => {
              onAdd(term, addCorrection ? correction : undefined);
              onClose();
              setTerm("");
              setCorrection("");
